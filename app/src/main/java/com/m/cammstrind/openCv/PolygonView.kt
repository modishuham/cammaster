package com.m.cammstrind.openCv
...
class PolygonView : FrameLayout {
    ...
    fun getListPoint(): ArrayList<Point> {
        ...
        listPoints.add(
            Point(
                pointer1!!.x.toDouble() + pointer1!!.width / 2,
                pointer1!!.y.toDouble() + pointer1!!.height / 2
            )
        )
        listPoints.add(
            Point(
                pointer2!!.x.toDouble() + pointer1!!.width / 2,
                pointer2!!.y.toDouble() + pointer2!!.height / 2
            )
        )
        listPoints.add(
            Point(
                pointer4!!.x.toDouble() + pointer1!!.width / 2,
                pointer4!!.y.toDouble() + pointer3!!.height / 2
            )
        )
        listPoints.add(
            Point(
                pointer3!!.x.toDouble() + pointer1!!.width / 2,
                pointer3!!.y.toDouble() + pointer4!!.height / 2
            )
        )
        return listPoints
    }
    ...
    private fun initPaint() {
        paint = Paint()
        paint!!.color = resources.getColor(R.color.material_deep_teal_200, null)
        ...
        override fun onTouch(v: View, event: MotionEvent): Boolean {
            ...
            when (eid) {
                ...
                MotionEvent.ACTION_UP -> {
                    var color = 0
                    if (isValidShape(points!!)) {
                        color = resources.getColor(R.color.material_deep_teal_200, null)
                    } else {
